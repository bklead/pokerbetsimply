...
namespace PokerBet.Utilits
{
    public class Ean13
    {
        ...
        private float _fWidth = 30f;
        private float _fHeight = 14.9f;
        private float _fFontSize = 6.0f;
        private float _fScale = 2.0f;
        ...
        public void DrawEan13Barcode(System.Drawing.Graphics g, System.Drawing.Point pt)
        {
            ...
            string sTemp = String.Format("{0}{1}{2}{3}",
                this.CountryCode,
            ...
            for (int i = 0; i < strbEAN13.Length; i++)
            {
                if (sTempUPC.Substring(i, 1) == "1")
                {
                    ...
                    if ((i > 12 && i < 55) || (i > 57 && i < 101))
                        ...
                        g.FillRectangle(brush, xPosition, yStart, lineWidth, height - fTextHeight + 0.6f);
                    else
                        // Draw a full line.
            ...
            float yPosition = yStart + (height - fTextHeight) + 1;

            // Draw 1st digit of the country code.
            g.DrawString(sTemp.Substring(0, 1), font, brush, new System.Drawing.PointF(xPosition - 1, yPosition));

            xPosition += (g.MeasureString(sTemp.Substring(0, 1), font).Width + 35 * lineWidth) -
                (g.MeasureString(sTemp.Substring(1, 6), font).Width);

            for (int i = 1; i <= 6; i++)
            {
                var nextchar = sTemp.Substring(i, 1);

                g.DrawString(nextchar, font, brush, new System.Drawing.PointF(xPosition, yPosition));

                xPosition += g.MeasureString(nextchar, font).Width - 0.4f;
            }

            //// Draw MFG Number.
            //g.DrawString(sTemp.Substring(1, 6), font, brush, new System.Drawing.PointF(xPosition, yPosition));

            xPosition += (8 * lineWidth);
            ...
            for (int i = 7; i <= 12; i++)
            {
                ...
                xPosition += g.MeasureString(nextchar, font).Width - 0.5f;
            }
        ...
        public System.Drawing.Bitmap CreateBitmap()
        {
            ...
            this.DrawEan13Barcode(g, new Point(1, 1));
            g.Dispose();
            return bmp;
        }
